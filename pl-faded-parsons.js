...
class ParsonsWidget {
  constructor(config) {
    ...
    const missing = ["solution", "solutionList", "main", "toolbar", "starterOrderStorage", "solutionOrderStorage", "solutionSubmissionStorage"].filter(f => this.config[f] == null).join(", ");
    if (missing) throw new Error(`ParsonsWidget config requires field(s) ${missing} to be non-null`);

    if (this.config.prettyPrint) {
      (window.prettyPrint || function() { console.error("prettify bindings missing!"); })();
    }

    $("form.question-form").submit(() => this.onSubmit());

    $(this.config.toolbar).find(`.widget-help`).popover({
      content: () =>
    ...
    $(this.config.toolbar).find(`.widget-copy`).on({
      click: () => {
        if (navigator.clipboard) {
          // Use the Clipboard API
          navigator.clipboard.writeText(this.asPlaintext())
            .catch((err) => {
    ...
    $(this.config.toolbar).find('.widget-dark').on({ click: () => ParsonsGlobal.toggleDarkmode(), });
    ...
    const solutionTray = $(widget.config.solutionList).sortable({
      ...
      update: widget.config.onSortableUpdate,
    ...
    if (widget.config.starter) {
      const starterTray = $(widget.config.starterList).sortable({
        ...
        stop: function (event, ui) {
          ...
          widget.addLogEntry({ type: "moveInput", target: ui.item[0].id }, true);
        },
      });
      solutionTray.sortable("option", "connectWith", starterTray);
    }

    widget.updateVertLines();

    $(widget.config.main).find("li.codeline").each(function (_, codeline) { // unstable idx because lines shift!
      ...
      const navigateBetweenTrays = (moveCodelineNotCursor, rightward) => {
        ...
        const codeboxIdx = codeboxes.toArray().findIndex(c => $(c).has(codeline).length)
        const k = (codeboxIdx + (rightward ? +1 : codeboxes.length - 1)) % codeboxes.length;
        const newTray = codeboxes.eq(k).find("ul.codeline-list").first();
        const parallelTarget = newTray.children().filter((_, e) => e.getBoundingClientRect().bottom > topPx).first();
        if (moveCodelineNotCursor) {
          ...
          const newFocus = parallelTarget.length ? parallelTarget : newTray.children().last();
          newFocus.focus();
        }
      }
      ...
      $(codeline).on({
        ...
        keydown: function (e) {
          ...
          if (e.key === "Tab") {
            e.preventDefault();
            const inTheStarterTray = $(widget.config.starter).has(codeline).length;
            if (inTheStarterTray) {
      ...
      inputs.each((i, input) => // note: because blanks are static, i is always the correct index into inputs.
        $(input).on({
          input: (e) => {
            ...
            if (e.key.startsWith("Arrow")) {
              if (!handleVerticalArrowKeys(e)) {
                ...
                if (input.selectionEnd == cursorPosition) {
                  if (e.key.endsWith("Left") && cursorPosition == 0 && i != 0) {
                    e.preventDefault();
                    inputs.eq(i - 1)
                      .focus()
                      .each((_j, inp) => { inp.setSelectionRange(inp.value.length, inp.value.length); });
                  }
                  if (e.key.endsWith("Right") && cursorPosition == input.value.length && i != n - 1) {
                    e.preventDefault();
                    inputs.eq(i + 1)
                      .focus()
                      .each((_j, inp) => { inp.setSelectionRange(0, 0); });
                  }
                }
              }
              return;
  ...
  codelineSummary(line, idx) {
    // this schema is used in pl-faded-parsons.py `read_lines`!
    return {
      content: this.codelineText(line),
      indent: this.codelineIndent(line),
      segments: this.codelineSegments(line),
      index: idx,
    };
  }
  autoSizeInput(el) {
  ...
  calculateCodeIndent(dist_in_px, elem) {
    ...
    let old_code_indent = this.codelineIndent(elem);
    let new_code_indent = old_code_indent + Math.floor(dist / this.config.xIndent);
  ...
  solutionCode() {
    ...
    for (const line of solution_lines) {
      ...
      const lineText = this.codelineText(line);
  ...
  codelineText(codelineElem) {
    ...
    const spaceCount = this.config.xIndent * this.codelineIndent(codelineElem);
    return " ".repeat(spaceCount) + elemClone[0].innerText;
  }
  asPlaintext() {
    const toText = lines => lines.map(line =>
      " ".repeat(this.config.xIndent * this.codelineIndent(line)) +
        this.codelineSegments(line).givenSegments.join("BLANK")
    );
    const lang = ($(this.config.main).attr("language") || "").toLowerCase();
    const commentPrefix = ["java", "c", "c++", "c#", "js", "javascript", "ts", "typescript"].includes(lang) ? "# ": "// " ;
    const starters = toText(this.getSourceLines()).map(s => commentPrefix + s);
  ...
  updateIndent(new_code_indent, elem, absolute = true) {
    ...
    let old_code_indent = this.codelineIndent(elem);
    if (!absolute) {
      new_code_indent += old_code_indent;
    }
    ...
    if (old_code_indent != new_code_indent && new_code_indent >= 0) {
      this.config.onSortableUpdate(
        {
          type: "reindent",
          content: this.codelineText(elem),
          old: old_code_indent,
          new: new_code_indent,
        },
      ...
      $(elem).css(
        "margin-left",
        this.config.xIndent * new_code_indent + "ch",
      );
  ...
  updateVertLines() {
    ...
    const [backgroundColor, tabStopColor] = ["var(--code-background)", "var(--pln-txt-color-faded)"];
    const [solidTabStops, dashedTabStop] =
      [ `linear-gradient(${tabStopColor}, ${tabStopColor}) no-repeat border-box, `.repeat(max_code_indent),
    ...
    $(this.config.solutionList).css({
      "background": ParsonsGlobal.uiConfig.showNextTabStop ? (solidTabStops + dashedTabStop) : solidTabStops.slice(0, -2),
  ...
  onSubmit() {
    const starterElements = this.getSourceLines().map((line, idx) => this.codelineSummary(line, idx));
    const $$ = selector => {
      const s = $(selector);
      if (!s.length) alert("Could not save student data!\nStorage missing at: " + selector);
      return s;
    };
    $$(this.config.starterOrderStorage).val(JSON.stringify(starterElements));

    const solutionElements = this.getSolutionLines().map((line, idx) => this.codelineSummary(line, idx));
    $$(this.config.solutionOrderStorage).val(JSON.stringify(solutionElements));

    $$(this.config.solutionSubmissionStorage).val(this.solutionCode().solution);
  }
  toggleDarkmode() {
    $(this.config.main).find(ParsonsGlobal.prettifyOutputClasses)
  ...
  updateAriaLabel(codeline, announce=true) {
    ...
    const lineNumber = 1 + $(codeline.parentElement).children().toArray().findIndex(e => e == codeline);
    const inStarterTray = $(this.config.starterList).has(codeline).length;
    const indentPrefix = inStarterTray ? "unused" : `indent ${this.codelineIndent(codeline)}`;
    const clone = $(codeline).clone();
    clone.find("input").each((_, inp) => inp.replaceWith(inp.value ? `full-blank(${inp.value})` : "empty-blank"));
...
/////////////////////////////////////////////////////////////////////////////////////////////
window.ParsonsGlobal = window.ParsonsGlobal || { // ensure singleton //
  makeLogger: false,
  widgets: [],
  prettifyOutputClasses: [
      "prettyprint", "linenums",
      "pln", "str", "kwd", "com", "typ", "lit", "dec", "var",
      "pun", "opn", "clo", "tag", "atn", "atv", "fun",
      "L0", "L1", "L3", "L4", "L5", "L6", "L7", "L8", "L9"
    ].map(cls => `.${cls}`).join(","),
  uiConfig: {
