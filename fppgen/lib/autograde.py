...
class AutograderConfig(ABC):
    ...
    @abstractmethod
    def info_json_update(
        self,
    ) -> Dict[str, Union[str, Dict[str, Union[bool, str, int]]]]:
        pass
    ...
    def clean_tests_dir(self, test_dir: Path | str) -> None:
        return
    ...
    def generate_server(
        ...
        *,
        no_ast: bool = False,
        tab: str = "    ",
    ) -> tuple[str, list[AnnotatedName], list[AnnotatedName]]:
        return (SERVER_DEFAULT, [], [])


autograders: Dict[str, type[AutograderConfig]] = {}
...
def register_autograder(extension: str = ""):
    ...
    def add_autograder(cls: type[AutograderConfig]) -> type[AutograderConfig]:
        autograders.update({with_dot: cls, without_dot: cls})
        return cls
...
def new_autograder_from_ext(extension: str) -> AutograderConfig:
    if extension not in autograders:
        if extension in ("fpp", ""):
            Bcolors.warn(
                'Autograder not specified! Add the "autograder" field '
...
class PythonAutograder(AutograderConfig):
    ...
    def populate_tests_dir(
        ...
        try:
            try:
                jsn = json.loads(test_region)
                success, test_file = True, make_test_file(jsn)
            except Exception:
            ...
            if log_details:
                Bcolors.fail(
                    "    * Generating tests from json failed with error:", e.msg
                )
...
class RubyAutograder(AutograderConfig):
    ...
    def clean_tests_dir(self, test_dir: Path | str) -> None:
        app_dir = Path(test_dir).parent / "app"
        print(
            f"Installing gems locally in `{app_dir}` with `{RUBY_SETUP_CMD}` ... ",
            end="",
        )
