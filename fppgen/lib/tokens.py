...
class Lexer:
    ...
    current_region: RegionDelim | None = None
    ...
    def put_region_delim(self, region_delim: str, lineno: int):
        if not len(region_delim):
            raise SyntaxError(
                "Regions must be named (ie ## test ##) " + self.format_ln(lineno)
            )

        if self.current_region:
            if region_delim != self.current_region.name:
                raise SyntaxError(
                    'Region "{}" began at {} before "{}" ended.'.format(
                        region_delim,
        ...
        try:
            ...
            for t in imported_regions.data:
                if t.region:
                    raise SyntaxError(
                        "Imported regions cannot contain regions. "
                        + self.format_ln(lineno)
                    )
    ...
    def finish(self) -> Tokens:
        if self.current_region:
            raise SyntaxError(
                'File ended before "{}" {} ended'.format(
                    self.current_region.name, self.format_ln(self.current_region.lineno)
                )
            )
...
def regex_chunk_lines(
    pattern: Pattern, txt: str, *, line_number=1
) -> Generator[
    tuple[int, Literal[False], str] | tuple[int, Literal[True], Match[str]], Any, None
]:
...
def lex(source_code: str, *, source_path: Path | None = None) -> Tokens:
